defmodule Crazy.MixProject do
  ...
  # invoke Crazy.start/2 when running on a target.
  def application("host") do
    [extra_applications: [:logger]]
  end

  def application(_target) do
    [mod: {Crazy.Application, []}, extra_applications: [:logger, :grovepi]]
  end
  ...
  defp deps(target) do
    [
      {:shoehorn, "~> 0.2"},
      {:nerves_runtime, "~> 0.4"},
      {:grovepi, ">= 0.0.1", path: "/Users/amos/workspace/opensource/grovepi"},
    ] ++ system(target)
  end
